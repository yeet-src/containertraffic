#!/usr/bin/env bash
# Spin up a few fake "containers" so container-traffic has named rows with mixed
# RED behavior to show. Each service runs in its own systemd cgroup scope
# (<name>.scope), which container-traffic attributes to a named row exactly like a
...
start_service() {
  ...
  # like a real container that both serves and makes requests. container-traffic
...
echo "traffic flowing. run container-traffic in another shell:  sudo yeet run -t ."
